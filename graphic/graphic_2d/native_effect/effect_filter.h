 ...
 *
 * @brief Declares the APIs for filter effects. It supports creating and managing various
 * filter effects, including frosted glass blur, brightness adjustment, grayscale
 * conversion, and color inversion. It also supports implementing rich image processing
 * effects through custom matrices, applicable to scenarios such as image editing,
 * photo beautification, and visual effects.
 *
 * You must call {@link OH_Filter_CreateEffect} and {@link OH_Filter_Release} in pairs
...
/**
 * @brief Creates an **OH_Filter** object to apply various filter effects (such as blur,
 * brightening, or grayscale) to an image, applicable to scenarios such as image editing,
 * album apps, and video processing.
 *
 ...
 * @param radius [in] Blur radius of the frosted glass effect. Value range: [0, +∞),
 *     in pixels. A value of 0 produces no blur effect; larger values produce stronger
 *     blur effects.
 * @return <ul>
 *         <li>{@link EffectErrorCode#EFFECT_SUCCESS} if the operation is successful.</li>
...
/**
 * @brief Creates a frosted glass filter effect and adds it to a filter effect chain.
 * It supports selecting the shader effect tile mode.
 *
 * @param filter [in] Filter pointer, which needs to be created through OH_Filter_CreateEffect. Cannot be NULL.
 * @param radius [in] Blur radius of the frosted glass effect. Value range: [0, +∞),
 *     in pixels. No blur effect is produced when the parameter value is 0.
 *     The larger the value, the stronger the blur effect.
 * @param tileMode [in] Shader effect tile mode. Different modes determine different
 *     processing methods for image edge areas. For details about the available options,
 ...
 *
 * @param filter [in] Filter pointer, which needs to be created through OH_Filter_CreateEffect. Cannot be NULL.
 * @param brightness [in] Brightness value of the brightening effect. Value range: [0, 1].
 *     The image remains unchanged when the value is 0, and becomes completely white
 *     when the value is 1.
 * @return <ul>
 *         <li>{@link EffectErrorCode#EFFECT_SUCCESS} if the operation is successful.</li>
 *         <li>{@link EffectErrorCode#EFFECT_BAD_PARAMETER} if filter is a null pointer
 *         or brightness is outside the value range [0, 1].</li>
 *         </ul>
 * @since 12
 * @version 1.0
 */
...
/**
 * @brief Creates a custom effect through a matrix and adds it to a filter effect chain,
 * applicable to scenarios that require specific color transformation effects
 * (such as color correction, hue adjustment, or color temperature adjustment).
 *
 * @param filter [in] Filter pointer, which needs to be created through OH_Filter_CreateEffect. Cannot be NULL.
 ...
 *
 * @param filter [in] Filter pointer used to create a bitmap, which needs to be created
 *     through OH_Filter_CreateEffect. Cannot be NULL.
