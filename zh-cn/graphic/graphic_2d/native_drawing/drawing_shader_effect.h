 ...
 *
 * @brief 声明与绘图模块中的着色器对象相关的函数，用于创建单色、线性渐变、径向渐变、扇形渐变、两点锥形渐变、
 * 图像和像素图等类型的着色器效果，按指定混合模式叠加两个着色器生成新的着色器效果；同时提供销毁着色器对象的函数，
 * 回收该对象占用的内存。
 * <br>本模块为单线程模型策略，需要调用方自行管理线程安全和上下文状态的切换。
 *
 * @kit ArkGraphics2D
 ...
 * @return 返回创建的着色器对象的指针{@link OH_Drawing_ShaderEffect}。如果返回NULL，表示创建失败；
 * 可能的原因是可用内存不足。
 * @since 12
 * @version 1.0
 */
 ...
 * <br>startPt、endPt、colors任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 ...
 * @param tileMode 着色器效果平铺模式类型。
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，startPt、endPt、
 * colors中任一为NULL，或tileMode超出枚举范围。
 * @since 11
 * @version 1.0
 */
...
/**
 * @brief 创建着色器，在两个指定点之间结合矩阵变换生成线性渐变。
 * <br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。
 * <br>startPt、endPt、colors任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 * <br>tileMode不在枚举范围内时返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE。
 *
 ...
 * @param size 表示颜色数组的长度（即元素个数），范围为[0, 256]，长度为0时，表示无渐变效果。
 * @param tileMode 着色器效果平铺模式类型。
 * @param matrix 指向矩阵对象{@link OH_Drawing_Matrix}的指针。如果矩阵指针为NULL，默认传入单位矩阵。
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，startPt、endPt、
 * colors中任一为NULL，或tileMode超出枚举范围。
 * @since 12
 * @version 1.0
 ...
 * <br>从起点到终点颜色从内到外进行圆形渐变（从圆心向边缘扩散）被称为径向渐变。
 * <br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。
 * <br>centerPt、colors任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 * <br>tileMode不在枚举范围内时返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE。
 *
 * @param centerPt 表示渐变的圆心。
 * @param radius 表示渐变的半径，需为非负数，单位为px。
 * @param colors 表示在径向上分布的颜色。
 * @param pos 表示colors中每个对应颜色的相对位置，数组长度需和colors保持一致。如果pos为NULL，颜色均匀分布在径向上。
 * @param size 表示颜色数组的长度（即元素个数），范围为[0, 256]，长度为0时，表示无渐变效果。
 * @param tileMode 着色器效果平铺模式类型。
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，centerPt、colors为NULL，
 * 或tileMode超出枚举范围。
 * @since 11
 * @version 1.0
 */
...
/**
 * @brief 创建着色器，在给定圆心和半径的情况下结合矩阵变换生成径向渐变。
 * <br>从起点到终点颜色从内到外进行圆形渐变（从圆心向边缘扩散）被称为径向渐变。
 * <br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。
 * <br>centerPt、colors任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 * <br>tileMode不在枚举范围内时返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE。
...
/**
 * @brief 创建着色器，在给定圆心的情况下结合矩阵变换生成扇形渐变。
 * <br>颜色从0°到360°渐变被称为扇形渐变。
 *
 * @param centerPt 表示渐变的圆心。
 * @param colors 表示在0°到360°范围内分布的颜色。
 * @param pos 表示colors中每个对应颜色的相对位置，数组长度需和colors保持一致。如果pos为NULL，颜色均匀分布在0°和360°之间。
 * @param size 表示颜色数组的长度（即元素个数），范围为[0, 256]，长度为0时，表示无渐变效果。
 * @param tileMode 着色器效果平铺模式类型。
 * @param matrix 指向矩阵对象的指针。如果矩阵指针为NULL，默认传入单位矩阵。
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，centerPt、colors为NULL，
 * 或tileMode超出枚举范围。
 * @since 20
 * @version 1.0
 ...
 * <br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。
 * <br>image、samplingOptions任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 * <br>tileX、tileY任意一个不在枚举范围内时返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE。
 *
 * @param image 指向图片对象{@link OH_Drawing_Image}的指针。
 * @param tileX 水平方向着色器效果平铺模式类型。
 * @param tileY 垂直方向着色器效果平铺模式类型。
 * @param samplingOptions 指向采样选项对象{@link OH_Drawing_SamplingOptions}的指针。
 * @param matrix 指向矩阵对象{@link OH_Drawing_Matrix}的指针。如果矩阵指针为NULL，默认传入单位矩阵。
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，image、
 * samplingOptions为NULL，或tileX、tileY超出枚举范围。
 * @since 12
 * @version 1.0
 */
 ...
 * @param samplingOptions 指向采样选项对象{@link OH_Drawing_SamplingOptions}的指针。
 * @param matrix 指向矩阵对象{@link OH_Drawing_Matrix}的指针。如果矩阵指针为NULL，默认传入单位矩阵。
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，pixelMap、
 * samplingOptions为NULL，或tileX、tileY超出枚举范围。
 * @since 20
 * @version 1.0
 */
...
/**
 * @brief 创建着色器，在给定两个圆之间生成锥形渐变。
 * <br>以两个圆为起止边界，颜色沿锥面方向进行渐变的效果被称为锥形渐变。
 * <br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。
 * <br>startPt、endPt、colors任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 * <br>tileMode不在枚举范围内时返回OH_DRAWING_ERROR_PARAMETER_OUT_OF_RANGE。
 *
 * @param startPt 表示渐变的起点圆心。
 * @param startRadius 表示渐变的起点半径，需为非负数，单位为px。
 * @param endPt 表示渐变的终点圆心。
 * @param endRadius 表示渐变的终点半径，需为非负数，单位为px。
 ...
 * <br>本接口会产生错误码，可以通过{@link OH_Drawing_ErrorCodeGet}查看错误码的取值。
 * <br>dst、src任意一个为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER；
 ...
 * @return 返回创建的着色器对象的指针。如果返回NULL，表示创建失败；可能的原因是可用内存不足，dst、src为NULL，
 * 或mode超出枚举范围。
 * @since 20
 * @version 1.0
 */
