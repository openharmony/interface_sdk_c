 ...
 *
 * @brief 提供排版行相关的接口，如获取指定位置处开始可以排版的字符个数等函数。
 *
 * @kit ArkGraphics2D
...
/**
 * @brief 创建一个排版行对象{@link OH_Drawing_LineTypography}的指针，排版行对象是保存文本内容以及样式的载体，可以用于计算单行排版信息。
 *
 * @syscap SystemCapability.Graphic.Graphic2D.NativeDrawing
 ...
 * @syscap SystemCapability.Graphic.Graphic2D.NativeDrawing
 * @param lineTypography 指向排版行对象{@link OH_Drawing_LineTypography}的指针，由{@link OH_Drawing_CreateLineTypography}获取。
 * @param startIndex 开始计算排版的起始位置（包括起始位置）。取值范围为[0, 文本字符总数）的整数。
 ...
 *     <br>可以先使用{@link OH_Drawing_LineTypographyGetLineBreak}获得合理的可用于进行排版的字符总数。如果该值设置为0，则返回自startIndex开始到文本末尾的内容。
