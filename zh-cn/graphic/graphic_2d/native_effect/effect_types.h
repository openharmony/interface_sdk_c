...
typedef enum {
    ...
    /** 发生了未被明确识别的错误，可能原因包括系统资源异常、API调用方式不当等。建议先检查API调用参数和系统资源状态。 */
 ...
 */
typedef enum {
    /** 边缘拉伸模式，如果着色器效果超出其原始边界，剩余区域使用着色器的边缘颜色填充。适用于需要平滑过渡到纯色背景的场景。 */
    CLAMP = 0,
    /** 平铺重复模式，在水平和垂直方向上重复着色器效果。适用于需要无缝平铺纹理的场景，如背景图案填充。 */
    REPEAT,
    /** 镜像平铺模式，在水平和垂直方向上重复着色器效果，交替镜像图像，以便相邻图像始终接合。适用于需要连续但避免生硬重复边缘的场景，如渐变背景。 */
    MIRROR,
    /** 贴花模式，仅在其原始边界内渲染着色器效果。适用于需要精确控制着色器边界的场景，边界外保持透明或原有内容。 */
    DECAL,
} EffectTileMode;
