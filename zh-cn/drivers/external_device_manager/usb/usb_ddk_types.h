...
typedef struct UsbDeviceDescriptor {
    ...
    /**
     * 端点零（控制端点）的最大包大小，只有8、16、32、64字节是合法的。
     */
...
/**
 * @brief 端点描述符，包含标准端点描述符和未做解析的描述符信息。
 *
 * @since 10
 * @version 1.0
 */
...
/**
 * @brief 接口描述符，包含标准接口描述符和端点描述符等信息。
 *
 * @since 10
 * @version 1.0
 */
typedef struct UsbDdkInterfaceDescriptor {
    ...
    /**
     * 该接口所包含的端点描述符，可为空指针表示无端点（不包含控制端点0）。
     */
...
typedef struct UsbDdkInterface {
    ...
    /**
     * USB接口的备用设置数组的指针，数组的长度由numAltsetting指定。
     */
    struct UsbDdkInterfaceDescriptor *altsetting;
} UsbDdkInterface;

/**
 * @brief 配置描述符，包含标准配置描述符和接口描述符等信息。
 *
 * @since 10
 * @version 1.0
 */
...
/**
 * @brief 请求管道，是USB数据传输请求的抽象，用于描述USB数据传输的基本配置参数，包括接口句柄、端点地址和超时时间。适用于需要进行USB数据传输的场景。
 *
 * @since 10
 * @version 1.0
 */
...
/**
 * @brief 设备内存映射，通过{@link OH_Usb_CreateDeviceMemMap}创建，使用映射后的缓冲区可提升数据传输性能。
 *
 * @since 10
 * @version 1.0
 */
typedef struct UsbDeviceMemMap {
    ...
    const size_t size;
    /**
     * 所使用的缓冲区的偏移量，默认为0，表示没有偏移。偏移从缓冲区地址address开始计算，offset和bufferLength之和必须小于等于缓冲区大小size。
     */
    uint32_t offset;
    /**
     * 所使用的缓冲区的长度，默认等于缓冲区大小 size，表示使用全部的缓冲区。offset和bufferLength之和必须小于等于缓冲区大小size。
     */
...
typedef enum {
    ...
    USB_DDK_NO_PERM = 201,
    /**
     * 非法参数。请检查传入参数的有效性，包括指针是否为空、数值范围是否合法等，在API version 16之前值为-2。
     */
    USB_DDK_INVALID_PARAMETER = 401,
    /**
     * 内存相关的错误，包括：内存不足、内存数据拷贝失败、内存申请失败等。请检查系统可用内存是否充足、内存操作参数是否正确，在API version 16之前值为-3。
     */
    ...
    /**
     * 非法操作，如在设备未初始化时调用接口、在错误状态下执行操作等。请检查DDK是否已初始化、接口句柄是否有效等，在API version 16之前值为-4。
     */
...
/**
 * @brief 设备ID数组，用于存放{@link OH_Usb_GetDevices}接口获取到的设备ID列表和设备数量。开发者申请设备ID数组，使用完结构体后需释放申请的内存，否则会造成资源泄漏。
 *
 * @since 18
 */
typedef struct Usb_DeviceArray {
    /**
     * 开发者申请好的设备ID数组首地址，申请的数组大小建议不超过128，以避免过度占用内存。
     */
    uint64_t* deviceIds;
    /**
