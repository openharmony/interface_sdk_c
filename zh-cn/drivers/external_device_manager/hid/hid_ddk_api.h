 ...
 *     <br>{@link HID_DDK_INVALID_OPERATION} 连接HID DDK服务失败。可能原因：DDK服务未初始化。请调用OH_Hid_Init完成初始化。
 *     <br>{@link HID_DDK_INVALID_PARAMETER} 参数校验失败。可能的原因：1. 入参hidDevice为空指针；
 *     <br>2. 入参hidEventProperties为空指针；3. properties的长度超过7；4. hidEventTypes的长度超过5；
 *     <br>5. hidKeys的长度超过100；6. hidAbs的长度超过26；7.hidRelBits的长度超过13；
 *     <br>8. hidMiscellaneous的长度超过6。
 *     <br>{@link HID_DDK_FAILURE} 设备数量达到最大值200。
 * @since 11
 * @version 1.0
 */
 ...
 *     <br>{@link HID_DDK_NO_PERM} 权限校验失败。请确保应用已声明ohos.permission.ACCESS_DDK_HID权限。
 *     <br>{@link HID_DDK_INVALID_OPERATION} 连接HID DDK服务失败或者调用方不是设备的创建者。请确保调用了OH_Hid_Init完成初始化，且调用方是设备的创建者。
 *     <br>{@link HID_DDK_INVALID_PARAMETER} 参数校验失败。可能的原因：1.设备ID小于0；2.入参length长度超过7；3.入参items为空指针。请检查参数有效性。
 *     <br>{@link HID_DDK_NULL_PTR} 对应设备的注入为空。请确保使用OH_Hid_CreateDevice返回的有效设备ID，且设备未被销毁。
 * @since 11
 * @version 1.0
 */
 ...
 *     <br>{@link HID_DDK_NO_PERM} 权限校验失败。请确保应用已声明ohos.permission.ACCESS_DDK_HID权限。
 *     <br>{@link HID_DDK_INVALID_OPERATION} 连接HID DDK服务失败或者调用方不是设备的创建者。请确保调用了OH_Hid_Init完成初始化，且调用方是设备的创建者。
 *     <br>{@link HID_DDK_NULL_PTR} 设备不存在。请确保使用OH_Hid_CreateDevice返回的有效设备ID，且设备未被销毁。
 * @since 11
 * @version 1.0
 */
 ...
 *     <br>{@link HID_DDK_NO_PERM} 权限校验失败。请确保应用已声明ohos.permission.ACCESS_DDK_HID权限。
 *     <br>{@link HID_DDK_INIT_ERROR} 初始化DDK失败。
 ...
 * @return {@link HID_DDK_SUCCESS} 操作成功。
 *     <br>{@link HID_DDK_NO_PERM} 权限校验失败。请确保应用已声明ohos.permission.ACCESS_DDK_HID权限。
 *     <br>{@link HID_DDK_INVALID_PARAMETER} 参数校验失败。可能原因：1. dev为空；
 *     <br>2. data为空；3. length为0；4. length超过{@link HID_MAX_REPORT_BUFFER_SIZE}；
 *     <br>5. bytesWritten为空。
 *     <br>{@link HID_DDK_INIT_ERROR} DDK未初始化，请确保已调用{@link OH_Hid_Init}完成初始化。
 *     <br>{@link HID_DDK_SERVICE_ERROR} 与DDK服务通信失败。可能原因：服务内部错误。请检查当前操作和设备状态。
 *     <br>{@link HID_DDK_IO_ERROR} I/O操作失败。可能原因：设备状态异常、数据传输错误等。请检查设备状态和参数。
 ...
 *     <br>{@link HID_DDK_NO_PERM} 权限校验失败。请确保应用已声明ohos.permission.ACCESS_DDK_HID权限。
 *     <br>{@link HID_DDK_INVALID_PARAMETER} 参数校验失败。可能原因：1. dev为空；
 *     <br>2. data为空；3. bufSize为0；4. bufSize超过{@link HID_MAX_REPORT_BUFFER_SIZE}；
 *     <br>5. bytesRead为空。
 *     <br>{@link HID_DDK_INIT_ERROR} DDK未初始化，请确保已调用{@link OH_Hid_Init}完成初始化。
 *     <br>{@link HID_DDK_SERVICE_ERROR} 与DDK服务通信失败。可能原因：服务内部错误。请检查当前操作和设备状态。
 *     <br>{@link HID_DDK_MEMORY_ERROR} 内存数据拷贝失败。
 *     <br>{@link HID_DDK_IO_ERROR} I/O操作失败。可能原因：设备状态异常、数据传输错误等。请检查设备状态和参数。
 *     <br>{@link HID_DDK_TIMEOUT} 读取超时。
 * @since 18
 */
 ...
 *     <br>{@link HID_DDK_SERVICE_ERROR} 与DDK服务通信失败。可能原因：服务内部错误。请检查当前操作和设备状态。
 *     <br>{@link HID_DDK_IO_ERROR} I/O操作失败。可能原因：设备状态异常、数据传输错误等。请检查设备状态和参数。
 *     <br>{@link HID_DDK_INVALID_OPERATION} 不支持此操作。
 * @since 18
 */
 ...
 *     <br>{@link HID_DDK_NO_PERM} 权限校验失败。请确保应用已声明ohos.permission.ACCESS_DDK_HID权限。
 *     <br>{@link HID_DDK_INVALID_PARAMETER} 参数校验失败。可能原因：1. dev为空；
 *     <br>2. buf为空；3. bufSize为0；4. bufSize超过{@link HID_MAX_REPORT_BUFFER_SIZE}；
 *     <br>5. bytesRead为空。
 *     <br>{@link HID_DDK_INIT_ERROR} DDK未初始化，请确保已调用{@link OH_Hid_Init}完成初始化。
 *     <br>{@link HID_DDK_SERVICE_ERROR} 与DDK服务通信失败。可能原因：服务内部错误。请检查当前操作和设备状态。
 *     <br>{@link HID_DDK_MEMORY_ERROR} 内存数据拷贝失败。
