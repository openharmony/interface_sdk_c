 ...
 *
 * @brief 声明ArkWeb网络协议栈错误码。该枚举定义了ArkWeb网络协议栈中可能出现的各种错误类型，覆盖网络连接、SSL/TLS、证书验证、HTTP/2、QUIC、缓存等多个方面的错误场景。
 * 开发者可以通过这些错误码快速定位网络请求失败的原因，便于进行故障诊断和错误处理。
 *
 * @kit ArkWeb
 * @library libohweb.so
...
typedef enum ArkWeb_NetError {
    ...
    /**
     * 异步IO操作尚未完成。这通常并不表示致命错误。这个错误将作为通知生成，以等待某个外部通知，表明IO操作最终已完成。
     */
    ...
    /**
     * 访问网络的权限被拒绝。这用于区分很可能是由防火墙导致的错误和其他访问被拒绝的错误。另请参阅ARKWEB_ERR_ACCESS_DENIED。
     */
    ...
    /**
     * SSL对等端向我们发送了一个致命的decrypt_error警告。这通常发生在对等端无法正确验证签名（在CertificateVerify或ServerKeyExchange中）
     * 或验证Finished消息时。
     */
    ...
    /**
     * 服务器响应了一个证书，其通用名称与主机名不匹配。这可能意味着：
     * 1. 攻击者已将我们的流量重定向到他们的服务器，并呈现了一个他们知道私钥的证书。
     */
    ARKWEB_ERR_CERT_COMMON_NAME_INVALID = -200,

    /**
     * 服务器以证书作为响应，但根据我们的时钟，该证书似乎尚未生效或已过期。这可能是以下情况之一：
     * 1. 攻击者提供了旧的证书，并设法获取了私钥。
     * 2. 服务器配置错误，没有提供有效的证书。
     * 3. 我们的时钟错误。
     */
    ...
    /**
     * 服务器回应了一个由我们不信任的机构签名的证书。这可能意味着：
     * 1. 攻击者已经将真实的证书替换为包含其公钥并由其同伙签名的证书。
     */
    ARKWEB_ERR_CERT_AUTHORITY_INVALID = -202,

    /**
     * 服务器返回的证书包含错误。此错误无法恢复。MSDN对此错误的描述如下："SSL证书包含错误。
     * "注意：目前尚不清楚这与ERR_CERT_INVALID有何不同。
     */
    ...
    /**
     * 此站点的安全证书的吊销信息不可用。这可能意味着：
     *
     * 1. 攻击者已破解证书中的私钥，并阻止了我们尝试查明证书已被吊销的操作。
     * 2. 证书未被吊销，但吊销服务器繁忙或不可用。
     */
