...
typedef enum ArkWeb_NetError {
    ...
    /**
     * @error 异步IO操作尚未完成。这通常并不表示致命错误。这个错误将作为通知生成，以等待某个外部通知，表明IO操作最终已完成。
     */
    ...
    /**
     * @error 对等端发送了SSL no_renegotiation警报消息。
     */
    ...
    /**
     * @error 访问网络的权限被拒绝。这用于区分很可能是由防火墙导致的错误和其他访问被拒绝的错误。另请参阅ARKWEB_ERR_ACCESS_DENIED。
     */
