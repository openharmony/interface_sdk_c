 ...
 * 调用此方法后，当前线程将被注册为IPC工作线程，参与处理来自其他进程的IPC请求，
 * 直到调用[OH_IPCSkeleton_StopWorkThread()]{@link oh_ipcskeleton_stopworkthread}退出线程池。
 *
 * - 仅在需要处理IPC请求的场景下调用。
 * - 调用后线程将阻塞等待处理IPC请求。
 * - 建议配合[OH_IPCSkeleton_StopWorkThread()]{@link oh_ipcskeleton_stopworkthread}使用以正确退出线程池。
 *
 * @syscap SystemCapability.Communication.IPC.Core
 * @since 12
 */
...
/**
 * @brief 当前线程退出IPC工作线程池，不再参与处理IPC请求。调用此方法后，当前线程将从IPC工作线程池中移除，不再被分配处理IPC请求的任务，线程可以执行其他操作。
 *
 * - 应在不再需要处理IPC请求时调用。
 * - 调用前需确保当前线程已通过[OH_IPCSkeleton_JoinWorkThread()]{@link oh_ipcskeleton_joinworkthread}加入线程池。
 * - 线程退出时应确保所有IPC事务处理完成。
 *
 * @syscap SystemCapability.Communication.IPC.Core
 * @since 12
 ...
 * 在跨进程的权限继承验证中确认最原始的权限来源。返回发起IPC调用的首个客户端的TokenId标识。
 *
 * - 仅在多级IPC调用场景中有意义。
 * - 在单级调用场景中，与[OH_IPCSkeleton_GetCallingTokenId()]{@link oh_ipcskeleton_getcallingtokenid}返回值相同。
 *
 * @syscap SystemCapability.Communication.IPC.Core
 * @return 返回首调者TokenId。
 * @since 12
...
/**
 * @brief 设置IPC工作线程池的最大线程数，控制IPC请求的并发处理能力。调用此方法后，IPC框架将按照设置的最大线程数管理工作线程，当并发IPC请求数超过线程数时，请求将排队等待。
 *
 * - 建议在应用启动时调用，避免运行时频繁修改。
 * - 此配置应在调用[OH_IPCSkeleton_JoinWorkThread()]{@link oh_ipcskeleton_joinworkthread}加入工作线程池之前完成，以确保线程池按配置大小工作。
 * - 此配置影响后续加入线程池的线程容量控制。
 * - 若无特殊诉求，不建议用户更改最大线程数。
 ...
 * 参数错误返回{@link OH_IPC_ErrorCode#OH_IPC_CHECK_PARAM_ERROR}（对应值：-1）；可能原因：maxThreadNum不在有效范围[1, 32]内，解决措施：检查并调整参数值；
 * 其它情况返回{@link OH_IPC_ErrorCode#OH_IPC_INNER_ERROR}（对应值：-2）。可能原因：IPC框架内部异常，解决措施：检查IPC服务状态或重启应用。
 * @since 12
 */
 ...
 * 在权限代理场景中以服务身份代为执行特权操作。调用此方法后，当前IPC上下文的身份凭证将切换为自身进程的凭证，同时返回原始调用方的凭证信息，用于后续恢复。
 *
 * - 必须在IPC请求处理上下文中调用。
 * - 返回的凭证信息需要由调用方妥善管理，使用后需释放。
 * - 必须与[OH_IPCSkeleton_SetCallingIdentity()]{@link oh_ipcskeleton_setcallingidentity}配对使用，在操作完成后恢复原始凭证。
 * - 避免长时间保持重置状态，可能影响权限校验。
 *
 * @syscap SystemCapability.Communication.IPC.Core
 * @param identity identity 用于存储调用凭证的内存地址，凭证中包含调用方的TokenId、UID和PID等身份信息，
 * 可用于后续通过{@link OH_IPCSkeleton_SetCallingIdentity()}恢复调用方身份。该内存由用户提供的分配器进行内存分配，用户使用完后需要主动释放。必须在IPC请求处理上下文中调用。不能为空。
 ...
 * @return 成功返回{@link OH_IPC_ErrorCode#OH_IPC_SUCCESS}（对应值：0）；
 * 参数错误返回{@link OH_IPC_ErrorCode#OH_IPC_CHECK_PARAM_ERROR}（对应值：-1）；可能原因：identity、len或allocator参数为空，解决措施：确保参数不为空；
 * 内存分配失败返回{@link OH_IPC_ErrorCode#OH_IPC_MEM_ALLOCATOR_ERROR}（对应值：-3）；可能原因：系统内存不足或内存分配器异常，解决措施：释放内存后重试或检查内存分配器；
 * 其它情况返回{@link OH_IPC_ErrorCode#OH_IPC_INNER_ERROR}（对应值：-2）；可能原因：IPC框架内部异常，解决措施：检查IPC服务状态或重启应用。
 * @since 12
 */
 ...
 * 在权限代理操作完成后还原调用方权限状态。
 *
 * - 调用此方法后，IPC上下文的身份凭证将恢复为之前通过[OH_IPCSkeleton_ResetCallingIdentity()]{@link oh_ipcskeleton_resetcallingidentity}
 * 保存的调用方凭证。
 * - 应在完成需要服务端身份的操作后立即调用。
 * - 身份凭证应正确管理，避免凭证泄露。
 *
