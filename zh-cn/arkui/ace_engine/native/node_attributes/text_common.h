...
typedef enum {
    /**
     * CJK（中文、日文、韩文）文本可以在任意2个字符间断行，而Non-CJK文本（如英文等）只能在空白符处断行。适用于对断行控制要求不高的通用场景。
     */
    ...
    /**
     * 对于Non-CJK的文本可在任意2个字符间断行，一行文本中有断点（如空白符）时，优先按断点换行。对于CJK的文本，换行效果与NORMAL效果保持一致。
     */
...
/**
 * @brief 定义文本菜单项，用于在文本选择菜单中表示单个菜单项。支持设置菜单项的标题、图标、启用状态等属性，适用于开发者需要自定义文本选择菜单内容、扩展菜单项功能的场景，帮助开发者灵活定制文本选择菜单。
 *
 * @since 22
 */
...
typedef enum {
    ...
    /**
     * 问问AI。对选中的文本提供AI问询能力。该菜单项依赖大模型能力才能生效。
     */
 ...
 * @param items 指向ArkUI_TextMenuItemArray对象的指针，该数组对象由系统内部创建并释放，在回调函数中开发者可以调用{@link OH_ArkUI_TextMenuItemArray_Insert}，
 *     {@link OH_ArkUI_TextMenuItemArray_Erase}进行数组修改。
 * @param userData 用户自定义数据指针，由开发者在注册回调时传入，在回调触发时原样回传给开发者，用于在回调函数中获取上下文数据。传参为null表示不传递自定义数据。
 * @since 22
 */
...
/**
 * @brief 定义装饰线样式，用于为文本添加装饰线效果，可设置装饰线的类型（如下划线、删除线、上划线等）、样式（如实线、虚线、波浪线等）和颜色。典型使用场景包括：为链接文本添加下划线、为已删除内容添加删除线、为重要文本添加上划线等。
 *
 * @since 24
 */
...
typedef enum {
    ...
    /** 圆角连接，文本描边拐角处显示为圆角。*/
    OH_ARKUI_STROKE_JOIN_STYLE_ROUND_JOIN,
    /** 斜角连接，文本描边拐角处显示为斜角。*/
    OH_ARKUI_STROKE_JOIN_STYLE_BEVEL_JOIN
...
typedef enum {
    ...
    /**
     * 如果设置为此类型且设置了其他类型时，选中文本后会显示与选中文本内容类型对应的菜单。如果设置为此类型但其他类型未设置时，选中文本后会显示此类型对应的菜单。例如，
     * 同时设置了文本识别类型为ARKUI_TEXT_SPAN_TYPE_TEXT、ARKUI_TEXT_SPAN_TYPE_DEFAULT的两个菜单，
...
typedef enum {
    ...
    /**
     * 如果设置为此类型且设置了其他类型时，触发其他类型的操作会显示对应类型的菜单。如果设置为此类型但其他类型未设置时，触发其他类型的操作会显示此类型对应的菜单。例如，
     * 同时设置了响应类型为ARKUI_TEXT_RESPONSE_TYPE_RIGHT_CLICK、ARKUI_TEXT_RESPONSE_TYPE_DEFAULT的两个菜单，
...
/**
 * @brief 创建文本输入框计数器的配置对象。当该对象不再使用时，请调用{@link OH_ArkUI_ShowCounterConfig_Dispose}销毁。
 *
...
/**
 * @brief 销毁由{@link OH_ArkUI_ShowCounterConfig_Create}创建的文本输入框计数器的配置对象。
 *
 * @param config 指向待销毁的文本输入框计数器的配置对象的指针。
 * @since 22
 */
void OH_ArkUI_ShowCounterConfig_Dispose(ArkUI_ShowCounterConfig* config);

/**
 * @brief 设置文本输入框未达到最大字符数时计数器的颜色。未通过该接口设置时，默认颜色为0x66182431，显示为灰色。
 *
 * @param config 指向文本输入框计数器的配置对象指针。使用前需先通过OH_ArkUI_ShowCounterConfig_Create()创建。
 * @param color 文本输入框未达到最大字符数时计数器的颜色，格式为0xARGB。
 * @since 22
 */
void OH_ArkUI_ShowCounterConfig_SetCounterTextColor(ArkUI_ShowCounterConfig* config, uint32_t color);

/**
 * @brief 设置文本输入框超出最大字符数时计数器的颜色。未通过该接口设置时，默认颜色为0x99FA2A2D，显示为红色。
 *
 * @param config 指向文本输入框计数器的配置对象指针。
 * @param color 文本输入框超出最大字符数时计数器的颜色，格式为0xARGB。
 * @since 22
 */
 ...
 *
 * @param config 指向文本输入框计数器的配置对象指针。
 * @return 返回文本输入框未达到最大字符数时计数器的颜色，格式为0xARGB，如果未通过{@link OH_ArkUI_ShowCounterConfig_SetCounterTextColor}接口设置计数器颜色，则返回0；
 *     否则返回已设置的颜色值。
 * @since 22
 */
 ...
 *
 * @param config 指向文本输入框计数器的配置对象指针。
 * @return 返回文本输入框超出最大字符数时计数器的颜色，格式为0xARGB，如果未通过{@link OH_ArkUI_ShowCounterConfig_SetCounterTextOverflowColor}接口设置计数器颜色，
 *     则返回0；否则返回已设置的颜色值。
 * @since 22
 */
...
/**
 * @brief 创建文本菜单项对象，用于自定义文本选择菜单或扩展系统菜单。适用于需要添加自定义菜单项（如分享到特定平台、自定义编辑操作等）的场景。当该对象不再使用时，请调用
 * {@link OH_ArkUI_TextMenuItem_Dispose}销毁。
 *
 * @return 指向文本菜单项对象的指针，用于在文本选择菜单中表示单个菜单项。
 * @since 22
 */
ArkUI_TextMenuItem* OH_ArkUI_TextMenuItem_Create();

/**
 * @brief 销毁由{@link OH_ArkUI_TextMenuItem_Create}创建的文本菜单项对象。
 *
...
/**
 * @brief 创建文本菜单扩展项对象，用于扩展文本编辑菜单功能。适用于需要为文本编辑组件添加自定义菜单项（如插入特殊字符、快速格式化等）的场景。当该对象不再使用时，请调用
 * {@link OH_ArkUI_TextEditMenuOptions_Dispose}销毁。
 *
 * @return 指向文本菜单扩展项对象的指针，用于扩展文本编辑菜单的功能。
 * @since 22
 */
ArkUI_TextEditMenuOptions* OH_ArkUI_TextEditMenuOptions_Create();

/**
 * @brief 销毁由{@link OH_ArkUI_TextEditMenuOptions_Create}创建的文本菜单扩展项对象。
 *
...
/**
 * @brief 创建自定义文本选择菜单对象，用于配置文本选择菜单的内容和行为。适用于需要完全自定义文本选择菜单（如替换默认菜单、添加应用专属操作）的场景。当该对象不再使用时，请调用
 * {@link OH_ArkUI_TextSelectionMenuOptions_Dispose}销毁。
 *
 * @return 指向自定义文本选择菜单对象的指针，用于菜单内容、样式和行为等自定义配置。
 * @since 22
 */
ArkUI_TextSelectionMenuOptions* OH_ArkUI_TextSelectionMenuOptions_Create();

/**
 * @brief 销毁由{@link OH_ArkUI_TextSelectionMenuOptions_Create}创建的自定义文本选择菜单对象。
 *
...
/**
 * @brief 创建文本内容基础控制器对象。当该对象不再使用时，请调用{@link OH_ArkUI_TextContentBaseController_Dispose}销毁。
 *
 * @return 指向文本内容基础控制器对象的指针，用于对文本组件进行内容控制，支持文本内容的获取、设置和更新等操作。
 * @since 23
 */
...
/**
 * @brief 创建一个装饰线样式对象，用于设置文本装饰线的类型、样式和颜色。适用于需要为文本添加下划线、删除线等装饰效果的场景，如富文本编辑器、超链接文本、价格标记等。当该对象不再使用时，请调用
 * {@link OH_ArkUI_DecorationStyleOptions_Destroy}销毁。
 *
 * @return 指向装饰线样式对象的指针，用于定义装饰线样式。
 * @since 24
 */
OH_ArkUI_DecorationStyleOptions* OH_ArkUI_DecorationStyleOptions_Create();

/**
 * @brief 销毁由{@link OH_ArkUI_DecorationStyleOptions_Create}创建的装饰线样式对象。
 *
