 ...
 * @param bufferSize 缓冲区大小。
 * @param writeLength 在返回ARKUI_ERROR_CODE_NO_ERROR时表示实际写入到缓冲区的字符串长度。
 *     <br>在返回ARKUI_ERROR_CODE_BUFFER_SIZE_ERROR时表示可以容纳目标的最小缓冲区大小。
 * @return 错误码。
 ...
 *         {@link ARKUI_ERROR_CODE_GET_INFO_FAILED} 查询信息失败，可能因为当前节点未挂载在页面下。
...
/**
 * @brief 获取当前节点所在的NavDestination组件在页面栈中的索引。
 *
 * @param node 指定的节点。
...
/**
 * @brief 注册一个回调函数，以便在下一帧渲染时执行，适用于Native侧在下一帧执行界面刷新或渲染相关任务的场景。不允许在非UI线程调用；如果检查到在非UI线程调用，程序会主动中止。
 *
 * @param uiContext {@link UIContext}对象指针，用以绑定实例。
 * @param userData 自定义事件参数，当事件触发时在回调参数中携带回来。
 * @param callback 自定义回调函数，用于在下一帧渲染时执行。
 * @param nanoTimestamp 帧信号的时间戳。
 * @param frameCount 帧号。
 * @return 错误码。
 *     <br>{@link ARKUI_ERROR_CODE_NO_ERROR} 成功。
 *     <br>{@link ARKUI_ERROR_CODE_CAPI_INIT_ERROR} CAPI初始化错误，请确认ArkUI Native接口运行环境已完成初始化后重试。
 *     <br>{@link ARKUI_ERROR_CODE_UI_CONTEXT_INVALID} uiContext对象无效，请检查uiContext是否为空或是否来自有效的UIContext对象。
 *     <br>{@link ARKUI_ERROR_CODE_CALLBACK_INVALID} 回调函数无效，请检查callback是否为空。
 * @since 18
 */
