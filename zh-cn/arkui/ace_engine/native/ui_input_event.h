 ...
 *
 * @brief 提供ArkUI在Native侧的输入事件定义，支持触摸、鼠标、轴、按键等事件类型及事件属性获取、事件冒泡控制、克隆事件处理等能力，适用于Native组件中识别和处理用户输入交互的场景，帮助开发者获取事件来源、位置、
 * 状态等信息并实现精细化交互控制。
 *
...
/**
 * @brief 定义触摸测试信息项句柄，用于表示触摸测试流程中的触摸测试信息项。
 *
 * @since 22
 */
...
/**
 * @brief 定义触摸测试信息项句柄数组，用于表示多个触摸测试信息项句柄。在触摸事件分发与测试过程中，可通过此数组类型统一管理和访问多个触摸测试结果，适用于需要同时处理多个触点测试信息的场景。
 *
 * @since 22
 */
 ...
 * {@link UI_TOUCH_EVENT_ACTION}，鼠标事件的action类型为{@link UI_MOUSE_EVENT_ACTION}。轴事件的action类型获取请使用
 * {@link OH_ArkUI_AxisEvent_GetAxisAction}接口，返回值类型为{@link UI_AXIS_EVENT_ACTION}，按键事件的action类型获取请使用
 * {@link OH_ArkUI_KeyEvent_GetType}接口，返回值类型为{@link ArkUI_KeyEventType}。
 *
 * @param event 表示指向当前UI输入事件的指针。
...
/**
 * @brief 获取UI输入事件的触发源类型。输入源为产生输入事件的真实物理设备，如触摸屏、鼠标等，由{@link UI_INPUT_EVENT_SOURCE_TYPE}定义，而输入工具为操作输入源设备来产生事件的工具，如手指、触控笔。
 * 在某些情况下两者可能容易发生混淆，比如当用户在操作鼠标时，鼠标既是输入源，也是输入工具。
 *
 * @note 对于按键事件，并不支持获取输入源类型，返回unknown。
 *
 * @param event 表示指向当前UI输入事件的指针。
 ...
 * {@link UI_INPUT_EVENT_TOOL_TYPE}枚举值定义。
 *
 * @note 对于按键事件，并不支持获取输入工具类型，返回unknown。
 *
 * @param event 表示指向当前UI输入事件的指针。
 ...
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中的序号。
 * @return 返回指向性输入事件中特定接触点相对于当前组件左上角的Y坐标，单位为px。如果参数异常则返回0.0f。
 * @since 12
 */
 ...
 * <br>所有类型事件，若传入的索引值小于0时为非法值。
 * <br>对于鼠标和轴事件，当索引值非0时为非法值。
 * <br>对于触摸事件，根据给定的索引获取特定接触点相对于当前组件左上角的X坐标。
 *
 * @param event 表示指向当前UI输入事件的指针。
 ...
 * <br>所有类型事件，若传入的索引值小于0时为非法值。
 * <br>对于鼠标和轴事件，当索引值非0时为非法值。
 * <br>对于触摸事件，根据给定的索引获取特定接触点相对于当前组件左上角的Y坐标。
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中目标触控点的索引。
 ...
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @return 返回相对于全局显示的Y坐标，单位为px。如果发生任何参数错误，例如传递的一个事件没有位置信息，则返回0.0f。
 * @since 20
 */
 ...
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中目标触控点的索引。有效值范围[0, {@link OH_ArkUI_PointerEvent_GetPointerCount()} - 1]
 * @return 返回相对于全局显示的Y坐标，单位为px。如果发生任何参数错误，则返回0.0f。
 * @since 20
 */
...
/**
 * @brief 获取当前触摸事件是左手点击触发还是右手点击触发。
 *
 * @note 仅在部分触控产品上有效。该值并非在按下时就能实时获取，在系统完成结果推断之前，皆默认返回NONE，因此请不要过度依赖该接口返回的结果。
 *
 * @param event 表示指向当前UI输入事件的指针。
 ...
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param historyIndex 表示历史事件数据列表的序号，必须小于{@link OH_ArkUI_PointerEvent_GetHistorySize}。
...
/**
 * @brief 从指向性输入事件（如触摸事件）中获取特定历史事件中的相对YZ平面的角度，取值的范围[-90, 90]，单位为deg，其中正值是向右倾斜。仅适用于支持倾角上报的触控笔操作产生的触控事件。
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中的序号。
 * @param historyIndex 表示历史事件数据列表的序号。
...
/**
 * @brief 从指向性输入事件（如触摸事件）中获取特定历史事件中的相对XZ平面的角度，取值的范围[-90, 90]，单位为deg，其中正值是向下倾斜。仅适用于支持倾角上报的触控笔操作产生的触控事件。
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中的序号。
 * @param historyIndex 表示历史事件数据列表的序号。
...
/**
 * @brief 从指向性输入事件（如触摸事件）中获取特定历史事件中的触屏区域的宽度。仅适用于手指操作产生的触控事件，这通常是一个圆形区域的半径。
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中的序号。
 * @param historyIndex 表示历史事件数据列表的序号。
...
/**
 * @brief 从指向性输入事件（如触摸事件）中获取特定历史事件中的触屏区域的高度。仅适用于手指操作产生的触控事件，这通常是一个圆形区域的半径。
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param pointerIndex 表示多点触控数据列表中的序号。
 * @param historyIndex 表示历史事件数据列表的序号。
...
/**
 * @brief 获取当前轴事件的垂直滚动轴的值。该值通常由鼠标滚轮或用户在触控板上双指竖向滑动产生。
 * <br>当通过鼠标滚轮触发时：
 * <br>当通过触控板双指竖向滑动触发时：
 * <br>通常情况下，垂直滚动轴事件只能驱动竖向的滑动手势响应。但当鼠标指针下命中的可滑动手势里可响应方向一致时，垂直滚动轴事件可以驱动这些滑动手势得到响应，即使这些手势所定义的方向是横向的。
 *
 * @param event 表示指向当前UI输入事件的指针。
...
/**
 * @brief 获取当前轴事件的水平滚动轴的值，该值通过在触控板上双指横向滑动产生。
 *
 * @note
 * 1. 上报的数值单位为PX，表示单次滚动增量，非滚动总量。
 * 2. 上报的数值不受用户配置的放大系数{@link OH_ArkUI_AxisEvent_GetScrollStep}影响。
 ...
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param axis 表示轴事件的轴类型{@link UI_AXIS_TYPE}。
 ...
 * @return 返回触摸测试模式配置结果的状态代码。返回0表示配置成功，返回非0表示配置失败。
 * @since 12
 */
 ...
 * @return 返回执行的状态代码。返回0表示设置成功，如果返回401，表示返回失败，可能的原因是参数异常，例如event是一个空指针，请检查event是否为空并传入有效的UI输入事件指针。
 * @since 12
 */
...
/**
 * @brief 设置是否阻止焦点轴事件冒泡，适用于组件已处理焦点轴事件后，不希望该事件继续传递给父组件或其他祖先组件的场景。
 *
 * @param event 表示指向当前UI输入事件的指针。
 * @param stopPropagation 表示是否阻止事件冒泡。true表示阻止事件冒泡，false表示不阻止事件冒泡。
 * @return 错误码。
 ...
 *     <br>**说明：** API版本26.0.0之前，返回值并非鼠标硬件的原始移动数据，而是原始数据缩小了X倍，X为系统的显示大小比例。API版本26.0.0开始，返回值为鼠标硬件的原始移动数据。
 * @since 15
 */
...
/**
 * @brief 基于原始事件指针创建克隆事件指针。仅对触摸事件有效。调用成功后，应在克隆事件使用完毕后调用OH_ArkUI_PointerEvent_DestroyClonedEvent()销毁该克隆事件指针，避免资源未释放。
 *
 * @param event ArkUI_UIInputEvent事件指针。
...
/**
 * @brief 销毁由OH_ArkUI_PointerEvent_CreateClonedEvent()创建的克隆事件指针。克隆事件使用完毕后应调用该接口释放资源。
 *
 * @param event ArkUI_UIInputEvent事件指针。
 * @return 返回结果代码。
 *     <br>如果操作成功，则返回{@link ARKUI_ERROR_CODE_NO_ERROR}。
...
/**
 * @brief 设置指向性克隆事件相对于当前组件左上角的X坐标和Y坐标。该接口应在调用OH_ArkUI_PointerEvent_CreateClonedEvent()创建克隆事件后使用；
 * 同系列SetClonedEvent接口均仅适用于克隆事件。
 *
 * @param event ArkUI_UIInputEvent事件指针。
...
/**
 * @brief 设置克隆指向性事件特定接触点相对于当前组件左上角的X坐标和Y坐标。只有通过{@link OH_ArkUI_PointerEvent_CreateClonedEvent}
...
/**
 * @brief 设置当前带有指向性的克隆输入事件的事件类型。只有通过{@link OH_ArkUI_PointerEvent_CreateClonedEvent}接口创建的ArkUI_UIInputEvent克隆事件指针才能使用此接口。
 *
 * @param event ArkUI_UIInputEvent事件指针。
...
/**
 * @brief 设置当前带有指向性的克隆输入事件的触摸点ID。只有通过{@link OH_ArkUI_PointerEvent_CreateClonedEvent}
 * 接口创建的ArkUI_UIInputEvent克隆事件指针才能使用此接口。
 *
 * @param event ArkUI_UIInputEvent事件指针。
...
/**
 * @brief 设置带有指向性的克隆输入事件特定接触点的触摸点ID。只有通过{@link OH_ArkUI_PointerEvent_CreateClonedEvent}
 * 接口创建的ArkUI_UIInputEvent克隆事件指针才能使用此接口。
 *
 * @param event ArkUI_UIInputEvent事件指针。
...
/**
 * @brief 转发克隆事件到特定节点。使用该接口前应先调用OH_ArkUI_PointerEvent_CreateClonedEvent()创建克隆事件，
 * 并可按需调用OH_ArkUI_PointerEvent_SetClonedEventLocalPosition()等接口设置事件属性；
 * 克隆事件使用完毕后应调用OH_ArkUI_PointerEvent_DestroyClonedEvent()销毁。
 *
 * @param node 目标节点。
 * @param event ArkUI_UIInputEvent事件指针。
 * @return 返回结果代码。
...
/**
 * @brief 调用该方法获取最近一次UIInput相关方法的执行情况。通常情况下不需要使用该方法，仅在调用UIInput相关方法后需要确认最近一次调用是否执行异常时使用。
 *
...
/**
 * @brief 从组件事件中获取惯性滚动轴事件，仅当用户在触摸板上使用双指执行满足系统惯性滚动判定条件的抛滑操作，且指针位置下存在注册了{@link NODE_ON_COASTING_AXIS_EVENT}事件的组件时，
 * 才能获取到有效事件。在从{@link ArkUI_NodeEvent}对象获取到{@link ArkUI_UIInputEvent}对象后调用此方法。
 * <br>惯性滚动轴事件仅在双指抛滑并离开触控板时触发，因此仅支持触控板。惯性滚动轴事件会在手指离开触控板后，根据抛滑速度产生轴值逐渐衰减的事件。由于刷新频率和性能的影响，当前事件轴值可能高于或低于上一个事件值。在惯性滚动轴事件期间，
 * 以下四种行为会中断事件，并立即收到{@link ARKUI_COASTING_AXIS_EVENT_PHASE_END}。
 *
 * @param event ArkUI_UIInputEvent事件指针。
...
/**
 * @brief 设置惯性滚动轴事件是否启用冒泡，默认禁止冒泡；适用于当前组件处理惯性滚动轴事件后，还需要将事件继续传递给祖先组件协同处理滚动效果的场景。
 *
 * @param event ArkUI_CoastingAxisEvent事件指针。
...
/**
 * @brief 获取触摸测试信息中的触摸测试信息项数组。
 *
 * @param info 指向触摸测试信息的指针。
 * @param array 输出参数，指向用于接收触摸测试信息项数组的指针，数组元素为触摸测试信息项，数组大小通过size参数返回。
 * @param size 输出参数，表示array中返回的触摸测试信息项数量。
 * @return 返回结果代码。
 *     <br>如果操作成功，则返回{@link ARKUI_ERROR_CODE_NO_ERROR}。
 *     <br>如果入参错误，则返回{@link ARKUI_ERROR_CODE_PARAM_INVALID}。
 * @since 22
 ...
 *
 * @param info 指向触摸测试信息项的指针。
 * @param buffer 调用者提供的缓冲区，用于接收获取到的子组件ID字符串。
...
/**
 * @brief 设置触摸测试策略，即组件及其子组件在命中测试过程中的行为方式，适用于自定义触摸命中结果、将触摸事件分发给指定子组件或控制兄弟节点是否继续参与命中测试的场景。
 *
...
/**
 * @brief 设置命中测试过程中需要作用的子组件ID，适用于自定义触摸测试结果并希望将触摸事件分发到指定子组件的场景。
 *
 * @param {pointer} info Indicates the pointer to a touch test info.
...
/**
 * @brief 获取表冠事件发生的时间。单位为ns。仅适用于入参event里存储的是表冠事件对象的场景。
 *
 * @param event 表示指向当前UI输入事件的指针。
...
/**
 * @brief 获取表冠事件发生的角速度。单位为°/s。仅适用于入参event里存储的是表冠事件对象的场景。
 *
 * @param event 表示指向当前UI输入事件的指针。
...
/**
 * @brief 获取表冠事件发生的旋转角度。单位为°。仅适用于入参event里存储的是表冠事件对象的场景。
 *
 * @param event 表示指向当前UI输入事件的指针。
...
/**
 * @brief 获取表冠事件发生的阶段。仅适用于入参event里存储的是表冠事件对象的场景。
 *
 * @param event 表示指向当前UI输入事件的指针。
...
/**
 * @brief 创建指定事件的克隆事件。该接口适用于触摸事件、鼠标事件和轴事件。调用成功后，可使用OH_ArkUI_ClonedEvent_SetXXX系列接口设置克隆事件属性，
 * 使用完毕后应调用OH_ArkUI_PointerEvent_DestroyClonedPointerEvent()销毁该克隆事件，避免资源未释放。
 *
 * @param event ArkUI_UIInputEvent事件指针。
 * @param clonedEvent ArkUI_UIInputEvent克隆事件指针。
 * @return 返回结果代码。
...
/**
 * @brief 创建新事件，不克隆现有事件。该接口适用于触摸事件、鼠标事件和轴事件。调用成功后，可使用OH_ArkUI_ClonedEvent_SetXXX系列接口设置事件属性，
 * 使用完毕后应调用OH_ArkUI_PointerEvent_DestroyClonedPointerEvent()销毁该事件，避免资源未释放。
 *
 ...
 *
 * @param event ArkUI_UIInputEvent事件指针。
 * @param toolType 克隆事件的工具类型。由{@link UI_INPUT_EVENT_TOOL_TYPE}枚举定义。
 * @return 返回结果代码。
 *     <br>如果操作成功，则返回{@link ARKUI_ERROR_CODE_NO_ERROR}。
 *     <br>如果入参错误，则返回{@link ARKUI_ERROR_CODE_PARAM_INVALID}。
 *     <br>如果输入的事件指针不是克隆事件指针，则返回{@link ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT}。
 ...
 *
 * @param event ArkUI_UIInputEvent事件指针。
 * @param fingerId 特定接触点的触摸点ID，可以通过{@link OH_ArkUI_PointerEvent_GetPointerId}接口获取。
 * @param pointerIndex 表示多点触控数据列表中目标触控点的索引。
 * @return 返回结果代码。
 *     <br>如果操作成功，则返回{@link ARKUI_ERROR_CODE_NO_ERROR}。
 *     <br>如果入参错误，则返回{@link ARKUI_ERROR_CODE_PARAM_INVALID}。
...
/**
 * @brief 设置克隆事件相对于YZ平面和XZ平面的倾斜角度。tiltX取值范围为[-90, 90]，其中正值表示向右倾斜；tiltY取值范围为[-90, 90]，其中正值表示向下倾斜。该接口适用于触摸事件。只有通过
 * {@link OH_ArkUI_PointerEvent_CreateClonedPointerEvent}和{@link OH_ArkUI_PointerEvent_CreatePointerEvent}
 * 接口创建的ArkUI_UIInputEvent对象才能使用此接口。
 *
 * @param event ArkUI_UIInputEvent事件指针。
 ...
 * {@link OH_ArkUI_PointerEvent_CreatePointerEvent}接口创建的ArkUI_UIInputEvent对象才能使用此接口。
 * @param event ArkUI_UIInputEvent事件指针。
 * @param pinchAxisScaleValue 捏合轴的缩放值，取值范围为[0, +∞)。
 * @return 返回结果代码。
 *     <br>如果操作成功，则返回{@link ARKUI_ERROR_CODE_NO_ERROR}。
 *     <br>如果入参错误，则返回{@link ARKUI_ERROR_CODE_PARAM_INVALID}。
 *     <br>如果输入的事件指针不是克隆事件指针，则返回{@link ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT}。
...
/**
 * @brief 使用指定的竞争策略将克隆事件发送到特定节点，适用于向目标节点注入克隆事件并需要控制注入事件与目标节点已有手势之间是否竞争的场景。只有通过
 * {@link OH_ArkUI_PointerEvent_CreateClonedPointerEvent}和{@link OH_ArkUI_PointerEvent_CreatePointerEvent}
 * 接口创建的ArkUI_UIInputEvent对象才能使用此接口。
 *
 * @param node 目标节点。
 ...
 * @since 24
 */
ArkUI_ErrorCode OH_ArkUI_PointerEvent_PostClonedEventWithStrategy(
    ArkUI_NodeHandle node, const ArkUI_UIInputEvent* event, ArkUI_CompetitionStrategy strategy);
