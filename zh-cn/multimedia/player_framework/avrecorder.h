 ...
 * 必须在{@link OH_AVRecorder_Create}和{@link OH_AVRecorder_Start}之间调用，调用成功之后进入AVRECORDER_PREPARED状态。<br>
...
/**
 * @brief 获取当前的录制参数。必须在{@link OH_AVRecorder_Prepare}之后调用。典型使用场景包括：录制开始前确认配置参数是否正确、在UI界面上展示当前录制设置信息等。<br>
 ...
 * @return AV_ERR_OK：执行成功。<br>
 * AV_ERR_INVALID_VAL：输入的recorder为nullptr或\，请查看日志获取详细信息*config不为nullptr。<br>
 ...
 * @return AV_ERR_OK：执行成功。<br>
 * AV_ERR_INVALID_VAL：输入的recorder为nullptr或输入的rotation不符合要求或更新视频旋转角度失败。
 * @syscap SystemCapability.Multimedia.Media.AVRecorder
 * @since 18
 */
...
/**
 * @brief 停止录制。必须在{@link OH_AVRecorder_Start}之后调用，调用成功之后进入AVRECORDER_STOPPED状态。<br>
 * 纯音频录制时，需要重新调用{@link OH_AVRecorder_Prepare}接口才能重新录制。<br>
 * 纯视频录制、音视频录制时，需要重新调用{@link OH_AVRecorder_Prepare}和{@link OH_AVRecorder_GetInputSurface}接口才能重新录制。<br>
 ...
 * 典型使用场景包括：录制完成后希望重新配置参数进行新一轮录制、录制出错后需要重置到初始状态重新开始等。<br>
 * 纯音频录制时，需要重新调用{@link OH_AVRecorder_Prepare}接口才能重新录制。<br>
...
/**
 * @brief 释放录制资源。必须在非AVRECORDER_RELEASED状态下调用，调用成功之后进入AVRECORDER_RELEASED状态。<br>
 ...
 * 典型使用场景包括：应用启动时查询设备支持的编码器、根据可用编码器选择合适的编码格式、在编码器选择界面展示可选列表等。<br>
 ...
 * @return AV_ERR_OK：执行成功。<br>
 * AV_ERR_INVALID_VAL：输入的recorder为nullptr或\*info不为nullptr。<br>
 ...
 * 必须在{@link OH_AVRecorder_Start}之前调用。<br>用户只能设置一个URI回调函数，当用户重复设置时，以最后一次设置的回调函数为准。
 * 
 * @param recorder 指向OH_AVRecorder实例的指针。
 ...
 * @return AV_ERR_OK：执行成功。<br>
 * AV_ERR_INVALID_VAL：输入的recorder为nullptr。<br>
 ...
 * @return AV_ERR_OK：执行成功。<br>
 * AV_ERR_INVALID_VAL：输入的参数recorder或amplitude为nullptr。<br>
 * AV_ERR_INVALID_STATE：不支持在当前状态下调用，应当在{@link OH_AVRecorder_Prepare}和{@link OH_AVRecorder_Stop}之间调用。<br>
 * AV_ERR_NO_MEMORY：内存不足，请释放资源后重试。<br>
 * AV_ERR_UNKNOWN：未知错误，请查看日志获取详细信息。
 * @syscap SystemCapability.Multimedia.Media.AVRecorder
 * @since 26.0.0
 */
 ...
 * @return AV_ERR_OK：执行成功。<br>
 * AV_ERR_INVALID_VAL：输入的参数recorder或metadata为nullptr，或者metadata中的值长度超过256字节。<br>
 * AV_ERR_INVALID_STATE：不支持在当前状态下调用，应当在{@link OH_AVRecorder_Prepare}和{@link OH_AVRecorder_Stop}之间调用。<br>
 * AV_ERR_NO_MEMORY：内存不足，请释放资源后重试。<br>
 * AV_ERR_UNKNOWN：未知错误，请查看日志获取详细信息。
 * @syscap SystemCapability.Multimedia.Media.AVRecorder
