 ...
 *
 * @param accessory [in] 打开流的音频配件。
 * @param stream [in] 新创建的输入流引用。使用此句柄通过Register...Callback注册回调。
 * @param streamInfo [in] 正在打开的流的音频流信息指针。此参数描述请求的流格式，
 *     配件可使用此信息配置数据通路。
 * @return <ul>
 *         <li>true：流打开成功。</li>
 *         <li>false：流打开失败。</li>
 ...
 *
 * @param accessory [in] 拥有该流的音频配件。
 * @param stream [in] 已启动的输入流引用。
 * @return <ul>
 *         <li>true：启动事件处理成功。</li>
 *         <li>false：启动事件处理失败。</li>
 *         </ul>
 * @since 26.0.0
 */
 ...
 *
 * @param accessory [in] 拥有该流的音频配件。
 * @param stream [in] 已停止的输入流引用。
 * @return <ul>
 *         <li>true：停止事件处理成功。</li>
 *         <li>false：停止事件处理失败。</li>
 *         </ul>
 * @since 26.0.0
 */
 ...
 *
 * @param accessory [in] 拥有该流的音频配件。
 * @param stream [in] 已释放的输入流（录音/采集流）引用。
 * @return <ul>
 *         <li>true：释放事件处理成功。</li>
 *         <li>false：释放事件处理失败。</li>
 *         </ul>
 * @since 26.0.0
 */
 ...
 * @param latency [out] 输出参数，返回时延值，单位为毫秒。
 * @return <ul>
 *         <li>true：获取时延成功。</li>
 *         <li>false：获取时延失败。</li>
 *         </ul>
 ...
 *
 * @param stream [in] 输入流句柄指针。
 * @param callback [in] 回调函数指针，不可为空。
 * @return <ul>
 *         <li>{@link AUDIOCOMMON_RESULT_SUCCESS}：函数执行成功。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM}：参数为空。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_ILLEGAL_STATE}：在
 *             {@link OH_AudioAccessory_OpenInputStreamCallback}外部调用或流已释放。</li>
 *         </ul>
 * @since 26.0.0
 */
 ...
 *
 * @param stream [in] 输入流句柄指针。
 * @param data [in] 音频数据缓冲区指针，不可为空。
 * @param dataSize [in] 音频数据大小，单位为字节，必须大于0。
 * @return <ul>
 *         <li>{@link AUDIOCOMMON_RESULT_SUCCESS}：函数执行成功。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM}：参数为空。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_FRAME_LENGTH_MISMATCH}：
 *             dataSize与当前流配置下20ms音频数据不对应。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_ILLEGAL_STATE}：流未启动或必须注册的流回调未全部注册。</li>
 ...
 *
 * @param stream [in] 输入流句柄指针。
 * @param writableSize [out] 输出参数，返回可写入的字节数。
 * @return <ul>
 *         <li>{@link AUDIOCOMMON_RESULT_SUCCESS}：函数执行成功。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM}：参数为空。</li>
 *         <li>{@link AUDIOCOMMON_RESULT_ERROR_ILLEGAL_STATE}：流已释放。</li>
 *         </ul>
 * @since 26.0.0
 */
