...
/**
 * @brief 将sample写入封装器。该接口必须在{@link OH_AVMuxer_Start}后，{@link OH_AVMuxer_Stop}前调用。调用者需要按info中的时间顺序将sample写入正确的音视频轨。
 *
 * @param muxer 指向OH_AVMuxer实例的指针。
...
/**
 * @brief 将sample写入封装器。该接口必须在{@link OH_AVMuxer_Start}后，{@link OH_AVMuxer_Stop}前调用。调用者需要按sample中的时间顺序将sample写入正确的音视频轨。
 *
 * @param muxer 指向OH_AVMuxer实例的指针。
 * @param trackIndex 数据对应的音视频轨的索引。
