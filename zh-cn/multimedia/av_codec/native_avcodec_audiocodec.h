 ...
 *
 * 一旦具有指定索引的缓冲区被提交给音频编解码器，该缓冲区将无法再次访问，直到再次收到{@link OH_AVCodecOnNeedInputBuffer}回调，收到相同索引时此缓冲区才可使用。
 *
...
/**
 * @brief 在设置的超时时间内，尝试查询对应音频编解码器可用的输出缓冲区的索引值。获取到索引值后，通过{@link OH_AudioCodec_GetOutputBuffer}接口获取的输出缓冲区使用完后，
 * 
 * 需要调用{@link OH_AudioCodec_FreeOutputBuffer}接口进行释放。释放后无法再次使用，长期不释放会阻塞编解码流程。注意：此接口仅可在音频编解码的同步模式中使用。
 *
 * @param codec 指向OH_AVCodec实例的指针。
...
/**
 * @brief 输入索引值，获取对应音频编解码器中该索引值对应的输出缓冲区。注意：此接口仅可在音频编解码的同步模式中使用。
 *
 * @param codec 指向OH_AVCodec实例的指针。
