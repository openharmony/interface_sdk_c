 ...
 *
 * @param mime mime类型描述字符串，请参阅{@link AVCODEC_MIMETYPE}。
 ...
 * @return 如果执行成功，则返回AV_ERR_OK，否则返回特定错误代码。
 * @deprecated since 11
...
/**
 * @brief 设置异步回调函数，使应用可以响应音频编码器生成的事件。在调用Prepare之前，必须调用此接口。
 *
 * @param codec 指向OH_AVCodec实例的指针。
...
/**
 * @brief 要配置音频编码器，通常需要配置编码后的音轨的描述信息。在调用Prepare之前，必须调用此接口。
 *
 * @param codec 指向OH_AVCodec实例的指针。
...
/**
 * @brief 调用此接口启动编码器，在Prepare成功后执行。启动后，编码器将开始上报OH_AVCodecOnNeedInputData事件。
 *
 * @param codec 指向OH_AVCodec实例的指针。
 * @return 如果执行成功，则返回AV_ERR_OK，否则返回特定错误代码。
 * @deprecated since 11
...
/**
 * @brief 获取编码器输出数据的描述信息。需要注意的是，返回值所指向的OH_AVFormat实例的生命周期需要调用者手动释放。
 *
 * @param codec 指向OH_AVCodec实例的指针。
...
/**
 * @brief 通知音频编码器已完成对index所对应缓冲区进行输入数据的填充。
 *
 * {@link OH_AVCodecOnNeedInputData}回调将报告可用的输入缓冲区和相应的索引值。一旦具有指定索引的缓冲区提交到音频编码器，则无法再次访问此缓冲区，直到再次收到
 * {@link OH_AVCodecOnNeedInputData}回调，收到相同索引时此缓冲区才可使用。
 *
