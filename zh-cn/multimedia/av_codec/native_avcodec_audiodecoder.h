...
/**
 * @brief 设置异步回调函数，使应用可以响应音频解码器生成的事件。在调用Prepare之前，必须调用此接口。
 *
 * @param codec 指向OH_AVCodec实例的指针。
...
/**
 * @brief 要配置音频解码器，通常需要配置从容器中提取的音频描述信息。在调用Prepare之前，必须调用此接口。
 *
 * @param codec 指向OH_AVCodec实例的指针。
...
/**
 * @brief 调用此接口启动解码器，在Prepare成功后执行。启动后，解码器将开始上报OH_AVCodecOnNeedInputData事件。
 *
 * @param codec 指向OH_AVCodec实例的指针。
 * @return 如果执行成功，则返回AV_ERR_OK，否则返回特定错误代码。
 * @deprecated since 11
...
/**
 * @brief 通知音频解码器已完成对index所对应缓冲区进行输入数据的填充。
 *
 * {@link OH_AVCodecOnNeedInputData}回调将报告可用的输入缓冲区和相应的索引值。一旦具有指定索引的缓冲区提交到音频解码器，则无法再次访问此缓冲区，直到再次收到
