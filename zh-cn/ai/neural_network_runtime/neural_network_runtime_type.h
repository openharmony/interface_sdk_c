 ...
 * 使用参数userData来查询希望获取的那次异步推理执行。
 * userData与调用异步推理{@link OH_NNExecutor_RunAsync}接口时传入的参数userData是一致的。 \n
 *
 * 使用参数errCode（{@link OH_NN_ReturnCode}类型）来获取该次异步推理的返回状态。 \n
 *
 * @param userData 异步推理执行的标识符，与调用异步推理{@link OH_NNExecutor_RunAsync}接口时传入的参数userData一致。
 * @param errCode 该次异步推理的返回状态（{@link OH_NN_ReturnCode}类型）。
 * @param outputTensor 异步推理的输出张量，与调用异步推理{@link OH_NNExecutor_RunAsync}接口时传入的参数outputTensor一致。
 * @param outputCount 异步推理输出张量的数量，与调用异步推理{@link OH_NNExecutor_RunAsync}接口时传入的参数outputCount一致。
 * @since 11
 * @version 1.0
 */
 ...
 * userData与调用异步推理{@link OH_NNExecutor_RunAsync}接口时传入的参数userData是一致的。
 *
 * @param userData 异步推理执行的标识符，与调用异步推理{@link OH_NNExecutor_RunAsync}接口时传入的参数 userData 一致。
 * @since 11
 * @version 1.0
 */
...
typedef enum {
     ...
     *
     * input，输入张量。
     * weight，卷积的权重，要求weight排布为[outChannel，kernelHeight，kernelWidth，1]，
     * outChannel = channelMultiplier * inChannel。
     * bias，卷积的偏置，是长度为[outChannel]的数组。在量化场景下，bias参数不需要量化参数，其量化
     *      版本要求输入 OH_NN_INT32类型数据，实际量化参数由input和weight共同决定。
     *
     * 参数：\n
     ...
     * 输出：\n
     *
     * output,n维Hardswish激活值，数据类型和shape和input一致。
     *
     * @since 12
     */
    OH_NN_OPS_HSWISH = 17,
     ...
     * 输出：\n
     *
     * output，一个4维张量，数据类型和input一致。shape由input，blockShape和paddings共同决定，
     *      假设input shape为[n,c,h,w]，则有 \n
     ...
     * newAxisMask，一个整数，用于新增维度。将newAxisMask转成二进制表示，如果binary(newAxisMask)[i]==1，
     *      则在第i维插入长度为1的新维度。
     ...
     *  topK，要返回最大值的数量，默认为1，当topK等于1时，将返回输入张量中最大值的索引；当topK大于1时，
     *      返回输入张量中前topK个最大值的索引。如果输入张量中有多个值相同且都是最大值，则返回其中任意一个。
    ...
    /**
     * 根据输入条件逐元素判定输出是从输入1还是输入2中选取值；当条件为true时，从输入1中取元素；当条件为false时，从输入2中取元素。
     *     当条件为张量时，三个输入的形状需要保持一致。
     *
     * 输入：\n
     *
    ...
    /**
     * 对输入张量做求余运算，input1和input2需要遵循数据类型转换的规则转成一致的数据类型；输入必须是两个张量或一个张量和一个标量。
     *      当输入是两个张量时，数据类型都不能是OH_NN_BOOL，可以广播为相同的形状；
     *      当输入是一个张量和一个标量时，标量输入只能是一个常量数值。
     *
     * 输入：\n
     *
    ...
    /**
     * 对目标检测模型的输出进行后处理。包括对模型输出的边界框、类别概率和分数进行解码，
     *      然后执行非极大值抑制（NMS）以去除重叠的边界框，最终输出检测结果。
     *
     * 输入：\n
     *
     * bbox，模型输出的边界框。
     ...
     * 输入：\n
     *
     * input,2维张量，形状为[batchSize，numClasses]，数据类型为OH_NN_FLOAT64、OH_NN_FLOAT32、OH_NN_FLOAT16。
     *
     * 参数：\n
     *
     * axis，指定进行计算的维度。
    ...
    /**
     * 逐元素计算两个张量的最小值，输入必须是两个张量，或一个张量和一个标量。当输入是两个张量是，数据类型不能同时是bool值，
     *       并且两个要能广播到相同的形状。当输入是一个张量和一个常量时，标量必须是常量。
     *
     * 输入：\n
     *
    ...
    /**
     * 计算输入张量在指定维度上的最大值，如果keepDims是false，输出张量的维度将会缩减；
     *      如果keepDims为true，输出张量的维度和输入张量保持不变。
     *
     * 输入：\n
     *
     * input，n维张量，其中n<8。
    ...
    /**
     * 计算输入张量在指定维度上的最小值，如果keepDims是false，输出张量的维度将会缩减；
     *      如果keepDims为true，输出张量的维度和输入张量保持不变。
     *
     * 输入：\n
     *
    ...
    /**
     * 计算输入张量在指定维度上的求和，如果keepDims是false，输出张量的维度将会缩减；
     *      如果keepDims为true，输出张量的维度和输入张量保持不变。
     *
     * 输入：\n
     *
    ...
    /**
     * 计算输入张量在指定维度上的L2范数，如果keepDims是false，输出张量的维度将会缩减；
     *      如果keepDims为true，输出张量的维度和输入张量保持不变。
     *
     * 输入：\n
     *
