 ...
 * <br>使用时需要判断数据是否返回空指针。只有当进度达到100%时，才会返回数据。
 *
 ...
 * <br>需要使用{@link OH_UDMF_GetDataElementAt}函数通过下标访问每个元素。
 * <br>此指针需要使用{@link OH_Udmf_DestroyDataArray}函数释放。
 ...
 * <br>若设置了目标路径，会将文件类型的数据进行拷贝到指定路径。回调中获取到的文件类型数据会被替换为目标路径的URI。
 * <br>若不设置目标路径，则不会执行拷贝文件操作。回调中获取到的文件类型数据为源端路径URI。
 * <br>若应用涉及复杂文件处理策略，或需要将文件拷贝在多个路径下时，建议不设置此参数，由应用自行完成文件拷贝相关处理。
 *
 ...
 * <br>当不再需要使用指针时，请使用{@link OH_UdmfDataLoadParams_Destroy}销毁实例对象，否则会导致内存泄漏。
 *
 ...
 * <br>当不再需要使用指针时，请使用{@link OH_UdmfDataLoadInfo_Destroy}销毁实例对象，否则会导致内存泄漏。
 *
