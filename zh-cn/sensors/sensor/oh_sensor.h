 ...
 *
 * @param infos 输出参数，双指针指向设备上所有传感器的信息。调用者不需要预先分配内存，函数内部会分配内存，调用者需要在使用完毕后负责释放内存。调用后，*infos指向传感器信息数组，请参考
 *     {@link Sensor_Info}。
 * @param count 输出参数，指向设备上传感器数量的指针。调用者传入指向uint32_t变量的指针即可，调用后count指向的变量会被设置为传感器的数量。
...
/**
 * @brief 订阅传感器数据。系统将以指定的频率向用户报告传感器数据。该订阅通过事件回调机制实现。使用场景：运动健康应用实时监测用户步数、心率等数据；游戏应用使用加速度传感器实现重力感应；导航应用使用陀螺仪传感器辅助定位。权限说明：
 * 订阅加速度传感器，需要申请ohos.permission.ACCELEROMETER权限；订阅陀螺仪传感器，需要申请ohos.permission.GYROSCOPE权限；订阅计步器相关传感器时，需要申请ohos.
 * permission.ACTIVITY_MOTION权限；订阅与健康相关的传感器时，比如心率传感器，需要申请ohos.permission.READ_HEALTH_DATA权限，否则订阅失败。订阅其余传感器不需要申请权限。
 *
 * @param id 输入参数，指向传感器订阅ID的指针。这是要订阅的传感器类型标识，调用者需要确保id指向的数据有效。请参考{@link Sensor_SubscriptionId}。该参数不应为NULL。
 * @param attribute 输入参数，指向订阅属性的指针，该属性用于指定数据报告频率。高频适合需要实时监测数据的场景（如游戏），低频适合省电且不需要高频数据的场景（如后台监测）。请根据实际需求选择合适的数据报告频率。请参考
 *     {@link Sensor_SubscriptionAttribute}。该参数不应为NULL。
 * @param subscriber 输入参数，指向订阅者信息的指针，该信息包含回调函数，用于报告传感器数据。调用者需要设置回调函数，系统会按照订阅属性中指定的频率调用该回调函数。请参考
 *     {@link Sensor_Subscriber}。该参数不应为NULL。注意：回调函数在传感器服务线程中执行，不应在回调中进行耗时操作或阻塞调用，否则可能影响传感器数据报告的实时性。
 * @return 返回{@link Sensor_Result}枚举值。主要枚举值如下：
...
/**
 * @brief 取消订阅传感器数据。该操作会停止传感器数据上报。取消订阅加速度传感器，需要申请ohos.permission.ACCELEROMETER权限；取消订阅陀螺仪传感器，需要申请ohos.permission.
 * GYROSCOPE权限；取消订阅计步器相关传感器时，需要申请ohos.permission.ACTIVITY_MOTION权限；取消订阅与健康相关的传感器时，需要申请ohos.permission.
 * READ_HEALTH_DATA权限，否则取消订阅失败。取消订阅其余传感器不需要申请权限。
 *
 * @param id 输入参数，指向传感器订阅ID的指针。这是要取消的订阅ID，必须与之前调用OH_Sensor_Subscribe时使用的订阅ID一致。请参考{@link Sensor_SubscriptionId}。
 *     该参数不应为NULL。
 * @param subscriber 输入参数，指向订阅者信息的指针，该信息包含回调函数，用于报告传感器数据。这个subscriber必须与之前调用OH_Sensor_Subscribe时使用的subscriber一致，
 *     用于唯一标识订阅者。取消订阅成功后，回调函数将不再被调用。请参考{@link Sensor_Subscriber}。该参数不应为NULL。
 * @return 返回{@link Sensor_Result}枚举值。主要枚举值如下：
