...
typedef struct Vibrator_FileDescription {
    ...
    int32_t fd;
    /**
     * 自定义振动序列的偏移地址，单位：B（字节）。用于指定从文件中读取振动序列的起始位置，offset≥0且小于文件大小。offset=0表示从文件开头读取，offset>0表示跳过前offset个字节。
     * offset超出文件大小时行为未定义。
     */
    int64_t offset;
    /**自定义振动序列的总长度，单位：B（字节）。表示从offset位置开始读取的字节数。length必须>0，且offset+length不能超过文件大小。length超出文件剩余大小时可能读取失败或产生未定义行为。*/
    int64_t length;
