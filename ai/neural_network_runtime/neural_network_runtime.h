 ...
 *
 * The {@link NN_QuantParam} instance needs to be released to avoid memory leak after it is
 * set to a {@link NN_TensorDesc}. \n
 *
...
/**
 * @brief Creates a model instance of the {@link OH_NNModel} type and uses other APIs provided by OH_NNModel to
 * construct the model instance.
 *
 * Before composition, call {@link OH_NNModel_Construct} to create a model instance. Based on the model topology,
 * call the {@link OH_NNModel_AddTensorToModel}, {@link OH_NNModel_AddOperation}, and {@link OH_NNModel_SetTensorData}
 * methods to fill in the data and operator nodes of the model, and then call {@link OH_NNModel_SpecifyInputsAndOutputs}
 * to specify the inputs and outputs of the model. After the model topology is constructed,
 * call {@link OH_NNModel_Finish} to build the model. \n
 *
 * After a model instance is no longer used, you need to destroy it by calling {@link OH_NNModel_Destroy}
 ...
 * This method is used to add tensors to a model instance based on the <b>tensorDesc</b> parameter with type of
 * {@link NN_TensorDesc}. {@link NN_TensorDesc} contains some attributes such as shape, format, data type and provides
 * corresponding APIs to access them. The order of adding tensors is specified by the indices recorded in the model.
 * The {@link OH_NNModel_SetTensorData}, {@link OH_NNModel_AddOperation}, and {@link OH_NNModel_SpecifyInputsAndOutputs}
 * methods specify tensors based on the indices. \n
 *
 * Neural Network Runtime supports inputs and outputs of the dynamic shape. When adding a data node with a dynamic
 * shape, you need to set the dimensions that support dynamic changes to <b>-1</b>. For example, if the shape of
 * a four-dimensional tensor is set to <b>[1, -1, 2, 2]</b>, the second dimension supports dynamic changes. \n
 *
 * @param model Pointer to the {@link OH_NNModel} instance.
 * @param tensorDesc Pointer to the {@link NN_TensorDesc} instance. The tensor descriptor specifies the attributes of
 *         the tensor added to the model instance.
 * @return Execution result of the function. If the operation is successful, <b>OH_NN_SUCCESS</b> is returned. If the
 *         operation fails, an error code is returned. For details about the error codes, see {@link OH_NN_ReturnCode}.
 * @since 11
 * @version 1.0
 ...
 * the operator parameters, inputs, and outputs are specified by <b>paramIndices</b>, <b>inputIndices</b>, and
 * <b>outputIndices</b> respectively. This method verifies the attributes of operator parameters and the number of
 * input and output parameters. These attributes must be correctly set when {@link OH_NNModel_AddTensorToModel} is
 * called to add tensors. For details about the expected parameters, input attributes,
 * and output attributes of each operator, see {@link OH_NN_OperationType}. \n
 *
 ...
 *
 * A tensor must be specified as the end-to-end inputs and outputs of a model instance. This type of tensor cannot
 * be set using {@link OH_NNModel_SetTensorData}. \n
 *
 * The index of a tensor is determined by the order in which the tensor is added to the model.
 * For details about how to add a tensor, see {@link OH_NNModel_AddTensorToModel}. \n
 *
 ...
 *
 * After the model topology is set up, call this method to indicate that the composition is complete. After this
 * method is called, additional composition operations cannot be performed. If {@link OH_NNModel_AddTensorToModel},
 * {@link OH_NNModel_AddOperation}, {@link OH_NNModel_SetTensorData}, and {@link OH_NNModel_SpecifyInputsAndOutputs}
 * are called, {@link OH_NN_OPERATION_FORBIDDEN} is returned. \n
 *
 ...
 *
 * If <b>model</b> or <b>*model</b> is a null pointer, this method only prints warning logs and
 ...
 * support status. The operator quantity for the array length is the same as that for the model instance. The memory
 * corresponding to this array is managed by Neural Network Runtime and is automatically destroyed after the model
 * instance is destroyed or this method is called again. \n
 *
 * @param model Pointer to the {@link OH_NNModel} instance.
 * @param deviceID Device ID to be queried, which can be obtained by using {@link OH_NNDevice_GetAllDevicesID}.
 * @param isSupported Pointer to the bool array. When this method is called, <b>(*isSupported)</b> must be a null
 *                    pointer. Otherwise, {@link OH_NN_INVALID_PARAMETER} is returned.
 * @param opCount Number of operators in a model instance, corresponding to
 *                the length of the <b>(*isSupported)</b> array.
 * @return Execution result of the function. If the operation is successful, <b>OH_NN_SUCCESS</b> is returned. If the
 *         operation fails, an error code is returned. For details about the error codes, see {@link OH_NN_ReturnCode}.
 * @since 9
 * @version 1.0
