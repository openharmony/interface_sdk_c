 ...
 *         {@link AV_ERR_IO_NETWORK_ACCESS_DENIED} if the client request parameters are incorrect or exceed the processing capacity.
 *         {@link AV_ERR_IO_RESOURE_NOT_FOUND}  cannot find available network resources.
 *         {@link AV_ERR_IO_SSL_CLIENT_CERT_NEEDED} if the server failS to verify the client certificate because the certificate is not carried,
 *           the certificate is invalid, or the certificate is expired.
 *         {@link AV_ERR_IO_SSL_CONNECT_FAIL} if the client fails to verify the server certificate because the certificate is not carried,
 *           the certificate is invalid, or the certificate is expired.
 ...
 *         {@link AV_ERR_IO_AUDIO_DEC_UNAVAILABLE} if the audio decoder is unavailable for handler incurrupt, or audio decoder subsystem abnormal.
 ...
 *         {@link AV_ERR_IO_AUDIO_ENC_UNAVAILABLE} if video decoder is unavailable for handler incurrupt, or video decoder subsystem abnormal.
 ...
 *         {@link AV_ERR_IO_VIDEO_DEC_UNAVAILABLE} if video decoder is unavailable for handler incurrupt, or video decoder subsystem abnormal.
 ...
 *         {@link AV_ERR_IO_VIDEO_ENC_UNAVAILABLE} if video encoder is unavailable for handler incurrupt, or video encoder subsystem abnormal.
 * @since 14
 */
